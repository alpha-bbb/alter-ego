...
class SpeakingStyle(BaseModel):
    ...
    emoji_usage: ExampleSet = Field(..., description="絵文字やスタンプの使用頻度と具体例")
...
class TemplateVal(BaseModel):
    ...
    knowledge: Optional[list[Knowledge]] = Field(default_factory=list, description="参照された知識")
    speaking_style: Optional[SpeakingStyle] = Field(None, description="話し方の分析結果")
    ...
    def __add__(self, other: Union["TemplateVal", dict]) -> "TemplateVal":
        ...
        if isinstance(other, dict):
            try:
                ...
                raise ValueError("渡された dict を TemplateVal に変換できませんでした") from e
        ...
        new_action_analysis = (
            self.action_analysis if self.action_analysis is not None else other.action_analysis
        )
        ...
        new_speaking_style = (
            self.speaking_style if self.speaking_style is not None else other.speaking_style
        )
...
class GraphState(BaseModel):
    ...
    talk_histories: list[TalkHistory] = Field(default_factory=list, description="会話履歴")
    talk_histories_markdown: MarkdownTalkHistories = Field(MarkdownTalkHistories(
        conversations= "",
        my_histories= "",
        other_histories= ""
    ), description="会話履歴のmarkdown形式")
    prompt_template: Annotated[str, operator.add] = Field("", description="プロンプトテンプレート")
    template_val: Annotated[TemplateVal, operator.add] = Field(TemplateVal(
        action_analysis=None,
        knowledge=None,
        speaking_style=None
    ), description="プロンプトテンプレートの変数")
    template_ready: bool = Field(False, description="プロンプトテンプレートが完成しているかどうか")
    final_responses: list[str] = Field(default_factory=list, description="生成された返答のリスト")
